...
```text
                  .agents/skills/ai-harness/SKILL.md   ← 唯一真实来源
                            ▲
        ┌───────────────────┼────────────────────┐
   Codex 原生扫描         Claude 目录软链        Cursor 复制（规划中 v0.7）
   （零成本，本就读        .claude/skills/         .cursor/skills/
    .agents/skills/）      → 软链指回源           （Cursor 不跟随软链）
```
...
| v0.7 | Cursor 适配（复制到 `.cursor/skills/`） | 规划中 |
| v0.8+ | Windsurf / Aider / Gemini 按需；分发打包 pipx；LICENSE/CI/CHANGELOG | 规划中 |
