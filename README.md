...
| 命令 | 作用 |
|---|---|
| `init [--profile core\|java-spring] [--agent codex,claude] [--with-hooks] [--with-commands] [--with-cursor] [--dry-run]` | 初始化分层骨架 + 装 skill（+ 可选 Stop hook / slash commands / Cursor 副本） |
| `upgrade [--apply] [--refresh-infra] [--with-hooks] [--with-commands] [--with-cursor]` | 已有库对齐到当前模板：补齐缺失文件 + bump schemaVersion（默认 dry-run，不覆盖用户内容） |
...
└── .agents/
    └── skills/ai-harness/
        └── SKILL.md          # ⭐ skill 唯一真实来源（Codex 原生扫此目录）
```
...
```text
                  .agents/skills/ai-harness/SKILL.md   ← 唯一真实来源
                            ▲
        ┌───────────────────┼────────────────────┐
   Codex 原生扫描         Claude 目录软链        Cursor 复制（--with-cursor）
   （零成本，本就读        .claude/skills/         .cursor/skills/
    .agents/skills/）      → 软链指回源           （Cursor 不跟随软链，复制+陈旧检测）
```

- **Codex**：`.agents/skills/` **正好是它的原生仓库级 skills 约定**，零成本即被发现（还跟随 symlink）。
- **Claude Code**：`.claude/skills/ai-harness` 是相对软链，指回 `.agents/`，改一处两处生效。
- **软链不可用的平台**（如未开开发者模式的 Windows）：`init` 自动回退为复制副本，`check` 提示「已退化、需手动同步」。
...
| `tests/test_harness_cli.py` | unittest（36 tests） | **工具代码** |
...
| v0.8 | Cursor 适配：`--with-cursor` 复制到 `.cursor/skills/` + 陈旧检测 | ✅ |
| v0.9+ | Windsurf / Aider / Gemini 按需；发布 PyPI；LICENSE✅/CI✅/CHANGELOG | 进行中 |
